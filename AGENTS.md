...
- Where each suite runs: `.github/workflows/ci.yml` — unit tests only, Python 3.13. `cloudbuild.yaml` — full suite in three sequential `pytest` invocations (test_core, then test_utils + test_config + test_sinks, then test_sync), preserving the order the GitHub workflow used historically. They share one Cloud Build step because each step starts a fresh container and only `/workspace` persists, so the dev-group install would otherwise be lost.
...
- The pull-request trigger runs as that dedicated service account **with** `_BQ_PROJECT` / `_BQ_DATASET` / `_GCS_BUCKET` set, so every pull request runs the integration tests against a real project. They were deliberately unset until 2026-09-03, because three defects (KPD-4933) made every non-denormalized write fail; all three were fixed together and verified by build `367cf3ae` (12 passed, whole suite green). Their shapes are worth knowing, because each was invisible to a green test suite:
