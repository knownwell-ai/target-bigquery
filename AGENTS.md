...
- Neither trigger sets `includeBuildLogs`. It was removed deliberately on 2026-09-03: with `INCLUDE_BUILD_LOGS_WITH_STATUS` the Cloud Build GitHub app inlines the whole build log into the check run on a **public** repo. The release build printed the Artifact Registry URL four times, and a failing merge build would publish the dataset name, the staging bucket and the CI service-account email through BigQuery 403/404 messages and `gcs_stage`'s URI logging. Read failures in the Cloud Build console instead, via the link on the check.
